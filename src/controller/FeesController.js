...
exports.feesDelete = catchAsync(async (req, res, next) => {
  try {
    ...
    const facultiesToUpdate = await Fees.find({ srNo: { $gt: deletedGrade.srNo } });
    if (facultiesToUpdate.length > 0) {
      await Faculty.updateMany({ srNo: { $gt: deletedGrade.srNo } }, { $inc: { srNo: -1 } });
    }
...
exports.feesEdit = catchAsync(async (req, res, next) => {
    try {
      ...
      if (!grade || first === undefined || second === undefined || third === undefined || fourth === undefined || total === undefined) {
        return res.status(400).json({
          status: false,
          message: "All fields are required!",
        });
